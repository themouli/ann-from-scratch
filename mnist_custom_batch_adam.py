...
class NeuralNetwork:
    ...
    def initialize_parameters(self):
        ...
        for l in range(1, L):
            ...
            self.m[f'dW{l}'] = np.zeros((self.layer_dims[l], self.layer_dims[l-1]))
            self.m[f'db{l}'] = np.zeros((self.layer_dims[l], 1))
            self.v[f'dW{l}'] = np.zeros((self.layer_dims[l], self.layer_dims[l-1]))
            self.v[f'db{l}'] = np.zeros((self.layer_dims[l], 1))
    ...
    def update_prameters(self, derivatives, learning_rate):
        ...
        for l in range(1, L):
            ...
            dW = derivatives[f'dW{l}']
            db = derivatives[f'db{l}']
            ...
            # bias correction
            m_hat_W = self.m[f'dW{l}'] / (1 - self.beta1**self.t)
            m_hat_b = self.m[f'db{l}'] / (1 - self.beta1**self.t)
            v_hat_W = self.v[f'dW{l}'] / (1 - self.beta2**self.t)
            v_hat_b = self.v[f'db{l}'] / (1 - self.beta2**self.t)

            # update parameters
            self.parameters[f'W{l}'] -= learning_rate * m_hat_W / (np.sqrt(v_hat_W) + self.epsilon)
            self.parameters[f'b{l}'] -= learning_rate * m_hat_b / (np.sqrt(v_hat_b) + self.epsilon)
